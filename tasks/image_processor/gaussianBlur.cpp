...
long double Blur::CountCoefficient(int x0, int x) const {
...
void Blur::ApplyFilter(Bmp& image, std::vector<double> args) {
    ...
    for (int y0 = 0; y0 < image.GetHeight(); ++y0) {
        for (int x0 = 0; x0 < image.GetWidth(); ++x0) {
            for (int i = x0 - s; i <= x0 + s; ++i) {
                int x = std::min(image.GetWidth() - 1, std::max(0, i));
    ...
    for (int y0 = 0; y0 < image.GetHeight(); ++y0) {
        for (int x0 = 0; x0 < image.GetWidth(); ++x0) {
            for (int i = y0 - s; i <= y0 + s; ++i) {
                int y = std::min(image.GetHeight() - 1, std::max(0, i));
